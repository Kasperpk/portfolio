...
Decision theory is a scientific analysis of decisions. Particularly decision theory studies decisions which is defined as a *choice between a set of alternatives or actions*. In a decision problem there is an agent or decision maker and the set of alternatives. In a world with perfect information the decision problem would not exist, as it would always be obvious which alternative maximizes utility, as the decision theorist would say. But that is just not the reality most people or businesses wake up to every morning. A lot of variables move in our environment which is both complex and changing. Decision theorists have developed a mathematical expression which can be used to understand the way agents make decisions and the role data and information plays. We will use this to understand more deeply what the value of data is.  
